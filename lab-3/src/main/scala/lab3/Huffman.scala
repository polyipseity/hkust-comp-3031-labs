...
trait Huffman extends HuffmanInterface:
  ...
  /**
   * In this lab, we are working with lists of characters. This function allows
  ...
  def times(chars: List[Char]): List[(Char, Int)] =
    chars.groupMapReduce(identity)(Function.const(1))(_ + _).toList.sortBy((char, _) => chars.indexOf(char))
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] =
    freqs.map(Leaf.apply).sortBy(_.weight)
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] =
    trees match {
      case left :: right :: next =>
        ...
        val (before, after) = next.span(weight(_) <= weight(fork))
        before ::: fork :: after
      case _ => trees
    }
  ...
  def until(done: List[CodeTree] => Boolean, merge: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): List[CodeTree] =
    @tailrec
    def impl(trees: List[CodeTree]): List[CodeTree] =
      if done(trees) then trees else impl(merge(trees))
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] =
    @tailrec
    def impl(currentTree: CodeTree, bits: List[Bit], ret: List[Char]): List[Char] =
      bits match {
        case head :: next => currentTree match {
          case Fork(left, right, chars, weight) => head match {
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] =
    table.find(_._1 == char).getOrElse(throw IllegalArgumentException())._2
  ...
  def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable =
    a.map((char, bits) => char -> (0 :: bits)) ::: b.map((char, bits) => char -> (1 :: bits))
